...
from platforms.forms import default_currencies_from_type, execution_system_choices
from currencies import currencies
from project.utils import get_accept_fields


GROUP_NAME_DEMO = 'demostandard_cfh'
GROUP_NAME_REAL = 'realstandard_cfh'


class CFHAccountForm(forms.Form):
    ...
    def save(self, profile=None):
        ...
        user = profile.user
        account = TradingAccount(platform_type='cfh', user=user, mt4_id=user.pk,
                                 group_name=GROUP_NAME_DEMO if self.account_type.is_demo else GROUP_NAME_REAL)
        if self.account_type.leverage_choices:
            account._leverage = self.cleaned_data.get('leverage', 50)
        password = account.api.account_create(account, initial_balance=self.cleaned_data.get('deposit', 0))
        if password:
            account.save()
        Contact.objects.filter(user=user).exclude(tags__contains=['cfh']).update(
            tags=Func(F('tags'), Value('cfh'), function='array_append'))
        if self.account_type.is_demo:
            ...
            notification.send([user], 'demo_pro_account_created',
                              {"password": password,
            ...
            notification.send([user], 'real_pro_account_created',
                              {"password": password,
                               'type': self.account_type,
                               "account": account.mt4_id,
                               "login": user.email})
