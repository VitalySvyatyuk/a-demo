...
class MenuItemAdmin(admin.ModelAdmin):
    ...
    change_form_template = "treemenus/change_form.html"
...
class MenuAdmin(admin.ModelAdmin):
    ...
    def get_object_with_change_permissions(self, request, model, obj_pk):
        ...
        if not self.has_change_permission(request, obj):
            raise PermissionDenied
        if obj is None:
            raise Http404('%s object with primary key %d does not exist.' % (model.__name__, int(obj_pk)))
        return obj
    ...
    def edit_menu_item(self, request, menu_pk, menu_item_pk):
        ...
        return menu_item_admin.change_view(request, menu_item_pk, extra_context={'menu': menu, 'show_delete': False})
