...
class SortHeaders:
    ...
    def get_query_string(self, params):
        ...
        always be present.
        """
        # self.request.REQUEST.copy() does not work here
        request_params = (self.request.GET or self.request.POST).copy()
        request_params.pop(ORDER_VAR, None)
        request_params.pop(ORDER_TYPE_VAR, None)
        
        params.update(self.additional_params)
        return '?%s&%s' % ('&'.join(['%s=%s' % (param, value) \
                                     for param, value in params.items()]), request_params.urlencode())
